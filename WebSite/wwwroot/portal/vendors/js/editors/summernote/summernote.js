...
(function (factory) {
  ...
  var buttonType = renderer.create('<div class="note-btn-group btn-group">');
  ...
  var HintPopover = function (context) {
    ...
    this.moveDown = function () {
      ...
      if ($next.length) {
        ...
        var $nextType = $current.parent().next();

        if (!$nextType.length) {
          $nextType = this.$content.find('.note-hint-group').first();
        }

        this.selectItem($nextType.find('.note-hint-item').first());
      }
    };
    ...
    this.moveUp = function () {
      ...
      if ($prev.length) {
        ...
        var $prevType = $current.parent().prev();

        if (!$prevType.length) {
          $prevType = this.$content.find('.note-hint-group').last();
        }

        this.selectItem($prevType.find('.note-hint-item').last());
      }
    };
    ...
    this.createType = function (idx, keyword) {
          ...
          if (bnd) {
            ...
            hints.forEach(function (hint, idx) {
              if (hint.match.test(keyword)) {
                self.createType(idx, keyword).appendTo(self.$content);
              }
            });
