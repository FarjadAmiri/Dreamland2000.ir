...
var Validation = {
    ...
    groupProperty: function (types, expression, comma) {
        ...
        while(expression.hasNext() && !result) {
            name = ValidationTypes.isAnyOfType(expression, types);
            if (name) {
                if (groups[name]) {
                    break;
                } else {
...
var ValidationTypes = {
    ...
    isAnyOfType: function(expression, types) {
        var args = types.split(" || "),
            i, len, found = false;
...
CSSLint.addRule({
    ...
    init: function (parser, reporter) {
        ...
        parser.addListener("endrule", function () {
                ...
                for (prop in compatiblePrefixes) {
                    if (compatiblePrefixes.hasOwnProperty(prop)) {
                        ...
                        if (CSSLint.Util.indexOf(variations, name.text) > -1) {
                            ...
                            if (CSSLint.Util.indexOf(propertyTypes[prop].actual, name.text) === -1) {
                                propertyTypes[prop].actual.push(name.text);
                                propertyTypes[prop].actualNodes.push(name);
                            }
                        }
                    }
                }
            ...
            for (prop in propertyTypes) {
                if (propertyTypes.hasOwnProperty(prop)) {
                    value = propertyTypes[prop];
                    full = value.full;
                    actual = value.actual;
